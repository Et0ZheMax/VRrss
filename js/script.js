...
document.addEventListener("DOMContentLoaded", function () {
  ...
  try {
    var elems = document.querySelectorAll('.select-account');
    elems.forEach(function(el) {
      el.addEventListener('click', function(e) {
        window.location.replace('/accounts?current=' + encodeURIComponent(el.getAttribute("data-value")))
      })})
  } catch (e) {
    console.log(e)
  }
  ...
  /*try {
    var elems = document.querySelectorAll('.select-session');
    elems.forEach(function(el) {
      el.addEventListener('click', function(e) {
        window.location.replace(el.getAttribute("data-value"))
      })})
  } catch (e) {
    console.log(e)
  }*/
  ...
  try {
    var elems = document.querySelectorAll('.select-emulator');
    elems.forEach(function(el) {
      el.addEventListener('click', function(e) {
        var xhr = new XMLHttpRequest();
        xhr.open('GET', "/pc/emulator?emulator=" + el.getAttribute("data-value"), false);
        xhr.onload = function() {
            if (xhr.status == 200) {
              location.reload();
            } else {
  ...
  try {
    function customSelect(elSelectNative, elSelectCustom) {
      ...
      elSelectCustomBox.addEventListener("click", function (e) {
        var disabled = elSelectCustom.classList.contains("disabled");
  ...
  try {
    const saveBtn = document.getElementById("accounts_save_btn")
    const currentAcc = document.getElementById("currentAccount")

    saveBtn.addEventListener("click", function(e){
      var elems = document.querySelectorAll('.fbots_settings')
      var intsettings = new Object()
      var stringsettings = new Object()

      for (var i = 0; i < elems.length; i++) {
        if (elems[i].type == 'checkbox') {
          intsettings[elems[i].name] = Number(elems[i].checked)
        } else if (elems[i].type == 'select-one') {
          intsettings[elems[i].name] = Number(elems[i].value)
        } else if (elems[i].type == 'number') {
          intsettings[elems[i].name] = Number(elems[i].value)
        } else {
          stringsettings[elems[i].name] = elems[i].value;
        }
      }
      ...
      var data = new Object();
      data["name"] = currentAcc.innerHTML
      data["int-settings"] = intsettings
      data["string-settings"] = stringsettings
...
*/
  try{
    const removeAccountBtn = document.getElementById("remove_account_btn");
    const removePopup = document.getElementById("remove-account");
    const overlay = document.querySelector('.overlay');
  ...
  try{
    const removeAccountBtn = document.getElementById("remove_session_btn");
    const removePopup = document.getElementById("remove-session");
    const overlay = document.querySelector('.overlay');
  ...
  try{
    const timeoutPopup = document.getElementById("timeout");
    const overlay = document.querySelector('.overlay');
    const btn = document.querySelectorAll('.open-timeout-popup');
  ...
  try {
    ...
    const normal_mode = document.getElementById("pc_session_area_normal");
    const edit_mode = document.getElementById("pc_session_area_edit");
    const normalBtn = document.querySelector(".pc__accounts__body__normal-mode");

    btn.addEventListener("click", function(e) {
      normal_mode.hidden = true
      edit_mode.hidden = false
    });

    normalBtn.addEventListener("click", function(e) {
      normal_mode.hidden = false
      edit_mode.hidden = true
    });
  } catch (e) {
    console.log(e);
  }
